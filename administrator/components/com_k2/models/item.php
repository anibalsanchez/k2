...
class K2ModelItem extends K2Model
{
    ...
    public function save($front = false)
    {
        ...
        if ($front && $row->id == null) {
            ...
            if (!K2HelperPermissions::canAddItem($row->catid)) {
                $app->enqueueMessage(Joomla\CMS\Language\Text::_('K2_YOU_ARE_NOT_ALLOWED_TO_POST_TO_THIS_CATEGORY_SAVE_FAILED'), 'error');
        ...
        // Author
        $row->created_by = $row->created_by ?: $user->get('id');
        if ($front) {
        ...
        $date = Joomla\CMS\Factory::getDate($row->created, $tzoffset);
        ...
        $date = Joomla\CMS\Factory::getDate($row->publish_up, $tzoffset);
        $row->publish_up = (K2_JVERSION == '15') ? $date->toMySQL() : $date->toSql();

        if (trim($row->publish_down) == Joomla\CMS\Language\Text::_('K2_NEVER') || trim($row->publish_down) === '') {
            ...
            $date = Joomla\CMS\Factory::getDate($row->publish_down, $tzoffset);
        ...
        if (K2Request::getBool('del_image')) {
            ...
            if (Joomla\CMS\Filesystem\File::exists(JPATH_ROOT.'/media/k2/items/src/'.$filename.'.jpg')) {
                Joomla\CMS\Filesystem\File::delete(JPATH_ROOT.'/media/k2/items/src/'.$filename.'.jpg');
            }

            if (Joomla\CMS\Filesystem\File::exists(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_XS.jpg')) {
                Joomla\CMS\Filesystem\File::delete(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_XS.jpg');
            }

            if (Joomla\CMS\Filesystem\File::exists(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_S.jpg')) {
                Joomla\CMS\Filesystem\File::delete(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_S.jpg');
            }

            if (Joomla\CMS\Filesystem\File::exists(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_M.jpg')) {
                Joomla\CMS\Filesystem\File::delete(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_M.jpg');
            }

            if (Joomla\CMS\Filesystem\File::exists(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_L.jpg')) {
                Joomla\CMS\Filesystem\File::delete(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_L.jpg');
            }

            if (Joomla\CMS\Filesystem\File::exists(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_XL.jpg')) {
                Joomla\CMS\Filesystem\File::delete(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_XL.jpg');
            }

            if (Joomla\CMS\Filesystem\File::exists(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_Generic.jpg')) {
                Joomla\CMS\Filesystem\File::delete(JPATH_ROOT.'/media/k2/items/cache/'.$filename.'_Generic.jpg');
            }
        ...
        if (isset($files['gallery']) && $files['gallery']['error'] == 0 && !K2Request::getBool('del_gallery')) {
            try {
                ...
                if ($handle->uploaded) {
                    ...
                    if (Joomla\CMS\Filesystem\Folder::exists($savepath.'/'.$row->id)) {
                        Joomla\CMS\Filesystem\Folder::delete($savepath.'/'.$row->id);
                    }

                    if (!JArchive::extract($savepath.'/'.$handle->file_dst_name, $savepath.'/'.$row->id)) {
                        $app->enqueueMessage(Joomla\CMS\Language\Text::_('K2_GALLERY_UPLOAD_ERROR_CANNOT_EXTRACT_ARCHIVE'), 'error');
                        $app->redirect('index.php?option=com_k2&view=items');
                    } else {
                        ...
                        while ($filename = readdir($galleryDir)) {
                            if ($filename !== '.' && $filename !== '..') {
                                ...
                                if (in_array($ext, $allowedExts)) {
                                    ...
                                    $safefilename = Joomla\CMS\Filesystem\File::makeSafe($file);
                    ...
                    if (Joomla\CMS\Filesystem\File::exists($archivePath)) {
                        Joomla\CMS\Filesystem\File::delete($archivePath);
                    }
        ...
        if (isset($files['video']) && $files['video']['error'] == 0 && !K2Request::getBool('del_video')) {
            ...
            $filename = $filename.'_'.Joomla\CMS\HTML\HTMLHelper::_('date', $filename_suffix, 'Ymd_Hi');
            $filename = preg_replace('/_+/', '_', $filename);

            $filetype = Joomla\CMS\Filesystem\File::getExt($files['video']['name']);

            if (!in_array($filetype, $validExtensions)) {
                $app->enqueueMessage(Joomla\CMS\Language\Text::_('K2_INVALID_VIDEO_FILE'), 'error');
                $app->redirect('index.php?option=com_k2&view=items');
            }

            $savepath = in_array($filetype, $videoExtensions) ? JPATH_ROOT.'/media/k2/videos' : JPATH_ROOT.'/media/k2/audio';

            Joomla\CMS\Filesystem\File::upload($files['video']['tmp_name'], $savepath.'/'.$filename.'.'.$filetype);
        ...
        if (K2Request::getBool('del_video')) {
            ...
            if (in_array($mediaType, $videoExtensions) && Joomla\CMS\Filesystem\File::exists(JPATH_ROOT.'/media/k2/videos/'.$mediaFile.'.'.$mediaType)) {
                Joomla\CMS\Filesystem\File::delete(JPATH_ROOT.'/media/k2/videos/'.$mediaFile.'.'.$mediaType);
            }

            if (in_array($mediaType, $audioExtensions) && Joomla\CMS\Filesystem\File::exists(JPATH_ROOT.'/media/k2/audio/'.$mediaFile.'.'.$mediaType)) {
        ...
        if ($params->get('showExtraFieldsTab') || $app->isClient('administrator')) {
            ...
            foreach ($csvFiles as $key => $file) {
                if ((bool) \Joomla\String\StringHelper::stristr($key, 'K2ExtraField_')) {
                    ...
                    if (!empty($csvFile) && Joomla\CMS\Filesystem\File::getExt($file['name'][0]) == 'csv') {
        ...
        if (is_array($attPost) && count($attPost)) {
            foreach ($attPost as $key => $attachment) { /* Use the POST array's key as reference */
                if (!empty($attachment['existing'])) {
                    $src = JPATH_SITE.'/'.Joomla\CMS\Filesystem\Path::clean($attachment['existing']);
                    $filename = basename($src);
                    $dest = $savepath.'/'.$filename;

                    if (Joomla\CMS\Filesystem\File::exists($dest)) {
                        $existingFileName = Joomla\CMS\Filesystem\File::getName($dest);
                        $ext = Joomla\CMS\Filesystem\File::getExt($existingFileName);
                        $basename = Joomla\CMS\Filesystem\File::stripExt($existingFileName);
                    ...
                    $attachmentToSave = Joomla\CMS\Table\Table::getInstance('K2Attachment', 'Table');
                    ...
                    try {
                        ...
                        if ($handle->uploaded) {
                            $handle->file_auto_rename = true;
                            $handle->file_new_name_body = Joomla\CMS\Filesystem\File::stripExt($filename);
                            $handle->file_new_name_ext = Joomla\CMS\Filesystem\File::getExt($filename);
        ...
        if ($front) {
            ...
            if (!K2HelperPermissions::canEditPublished($row->catid) && !K2HelperPermissions::canPublishItem($row->catid) && $newPublishedState) {
                $app->enqueueMessage(Joomla\CMS\Language\Text::_('K2_YOU_DONT_HAVE_THE_PERMISSION_TO_PUBLISH_ITEMS'), 'notice');
            }
        }
        ...
        if (Joomla\CMS\Filesystem\File::exists($file)) {
            ...
            if (!empty($tagReplace) && is_array($tagReplace)) {
                foreach ($tagReplace as $name => $embed) {
                    if (str_contains($embed, '<iframe') || str_contains($embed, '<script')) {
                        $providers[] = $name;
                    }
                }
            }
    ...
    public function download()
    {
        ...
        if (Joomla\CMS\Filesystem\File::exists($file)) {
            ...
            echo Joomla\CMS\Language\Text::_('K2_FILE_DOES_NOT_EXIST');
        }
    ...
    public function deleteAttachment()
    {
        ...
        $row = Joomla\CMS\Table\Table::getInstance('K2Attachment', 'Table');
        $row->load($id);

        $path = $params->get('attachmentsFolder', null);
        $savepath = is_null($path) ? JPATH_ROOT.'/media/k2/attachments' : $path;

        if (Joomla\CMS\Filesystem\File::exists($savepath.'/'.$row->filename)) {
            Joomla\CMS\Filesystem\File::delete($savepath.'/'.$row->filename);
        }
    ...
    public function resetHits()
    {
        ...
        $app->enqueueMessage(Joomla\CMS\Language\Text::_('K2_SUCCESSFULLY_RESET_ITEM_HITS'));
        $app->redirect($url);
    }
    ...
    public function resetRating()
    {
        ...
        $app->enqueueMessage(Joomla\CMS\Language\Text::_('K2_SUCCESSFULLY_RESET_ITEM_RATING'));
        $app->redirect($url);
    }
    ...
    public function checkSIG()
    {
        ...
        return (bool) (Joomla\CMS\Filesystem\File::exists($check));
    }
    ...
    public function cleanText($text)
    {
        if (version_compare(JVERSION, '2.5.0', 'ge')) {
            $text = Joomla\CMS\Component\ComponentHelper::filterText($text);
            ...
            $config = Joomla\CMS\Component\ComponentHelper::getParams('com_content');
            $user = Joomla\CMS\Factory::getUser();
            $gid = $user->get('gid');
